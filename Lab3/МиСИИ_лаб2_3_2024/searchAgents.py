    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWalls:
                ...
                nextStateCorners = stateCorners
                if (nextX, nextY) in self.corners and (nextX, nextY) not in stateCorners:
                    # Create a new independent tuple based on the stateCorners tuple
                    nextStateCorners = stateCorners + (nextStateCrds, )
                successors.append(((nextStateCrds, nextStateCorners), action, 1))
